...
public class GetUserTracksChart extends AbstractDzPagingIdMethod<Track, UserService> {
    public GetUserTracksChart(UserService deezerService, Gson gson, long objectId) {
        super(deezerService, gson, objectId);
    }

    @Override
    public CompletableFuture<Page<Track, DzPagingMethod<Track>>> executeAsync() {
        return deezerService.getUserTracksChartAsync(objectId, getQueryParams());
    }

    @Override
    public String toString() {
        return "/user/" + objectId + "/charts/tracks" + getQueryParams();
    }
}
