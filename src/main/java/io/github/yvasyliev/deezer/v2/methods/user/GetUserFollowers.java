...
public class GetUserFollowers extends AbstractDzPagingIdMethod<User, UserService> {
    public GetUserFollowers(UserService deezerService, Gson gson, long objectId) {
        super(deezerService, gson, objectId);
    }

    @Override
    public CompletableFuture<Page<User, DzPagingMethod<User>>> executeAsync() {
        return deezerService.getUserFollowersAsync(objectId, getQueryParams());
    }

    @Override
    public String toString() {
        return "/user/" + objectId + "/followers" + getQueryParams();
    }
}
